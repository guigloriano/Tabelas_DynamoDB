...
ND <- read_ods("C:\\Users\\Guilherme\\Desktop\\Enviar Ricardo\\85_105.ods")
...
for(i in 1:length(id.fi)){
  ...
  EQMi[i+84] <- mean((Dio - Dip)^2)
  #  EQMi[i] 
  
  EQMi[i+84] <- round(EQMi[i+84], 4)
...
nome_arquivo <- paste ("energia_", i+84, ".png", sep="")
...
sm <- seq(1,length(id.fi)+84)
plot(sm, eqm2, type="l", xlim=c(0,106),  ylim=c(0,0.03), ylab="EQM", xlab="Dia")
points(sm, eqm2, pch=19)

media_eqm_graf <- mean(eqm2)
