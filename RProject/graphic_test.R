...
#for(i in 1:length(names)){ 
  assign(names[2],read.csv(names[2],skip=1, header=TRUE))
  
  dataset <- readr::read_csv(names[2], col_types = cols(hora_minuto = col_character()))
...
#    theme(axis.text.x = element_text(angle = 90))
  ...
  p <- ggplot(data = dados_graf, mapping = aes(x = hora_minuto)) + 
    ...
    geom_line(aes(y=P_DC/100, color="P_DC", group=2)) + 
  #  geom_line(aes(y=pm1_concentracao, color="pm1_concentracao", group=3)) + 
  #  geom_line(aes(y=vento_dir/2, color="vento_dir", group=4)) + 
  #  geom_line(aes(y=preciptacao, color="preciptacao", group=4)) + 
    
    geom_line(aes(y=irradiacao/10, color="irr_est", group=2)) + 
    
    geom_line(aes(y=IRR/10, color="irr_inv", group=2, )) +
  ...
  fileDest <- paste(pathDest,  "TESTE2__", dia, ".png", sep = "")
